        ...
        <div className="min-h-screen bg-[#0f172a] pt-[calc(4rem+env(safe-area-inset-top,0px))] lg:pt-[calc(4.375rem+env(safe-area-inset-top,0px))]">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
                <div className="text-center mb-8 sm:mb-12">
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white tracking-tight mb-3">Contact Us</h1>
                    <p className="text-gray-400 text-sm sm:text-base max-w-xl mx-auto">Have questions? We'd love to hear from you. Send us a message and we'll respond as soon as possible.</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                    ...
                    <div className="space-y-4 sm:space-y-6">
                        {[
                            ...
                            <div key={item.title} className="flex gap-4 bg-[#1e2235] rounded-xl p-4 sm:p-5 hover:bg-white/5 transition-all duration-200">
                                <span className="text-2xl mt-0.5">{item.icon}</span>
                                <div>
                                    <h3 className="text-white font-bold text-sm mb-1">{item.title}</h3>
                                    {item.lines.map((line, i) => <p key={i} className="text-gray-400 text-xs sm:text-sm">{line}</p>)}
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="w-full max-w-lg mx-auto lg:mx-0">
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            <div>
                                <label htmlFor="name" className="block text-gray-400 text-xs uppercase tracking-wider font-semibold mb-1.5">Name</label>
                            ...
                            <div>
                                <label htmlFor="email" className="block text-gray-400 text-xs uppercase tracking-wider font-semibold mb-1.5">Email</label>
                            ...
                            <div>
                                <label htmlFor="message" className="block text-gray-400 text-xs uppercase tracking-wider font-semibold mb-1.5">Message</label>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                <button type="submit" disabled={isSubmitting} className="flex-1 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold rounded-xl text-sm hover:opacity-90 transition-all duration-200 border-none cursor-pointer disabled:opacity-50 active:scale-[0.98]">
                                    {isSubmitting ? 'Sending...' : 'Send Message'}
                                </button>
                                <button type="button" onClick={resetForm} disabled={isSubmitting} className="py-3 px-6 bg-transparent border-2 border-gray-700 text-white font-bold rounded-xl text-sm hover:border-gray-500 transition-all duration-200 cursor-pointer disabled:opacity-50">
                                    Reset
                                </button>
                            </div>
